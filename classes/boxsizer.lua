...
-- define the class:
local BoxSizer = class 'BoxSizer' : subclassof 'Object' : global(false)

enum -- wxStretch
{
    STRETCH_NOT = wx.wxSTRETCH_NOT, 
    SHRINK      = wx.wxSHRINK, 
    GROW        = wx.wxGROW, 
    EXPAND      = wx.wxEXPAND, 
    SHAPED      = wx.wxSHAPED, 
    ...
    FIXED_MINSIZE                 = wx.wxFIXED_MINSIZE, 
    RESERVE_SPACE_EVEN_IF_HIDDEN  = wx.wxRESERVE_SPACE_EVEN_IF_HIDDEN 
}
...
enum -- wxOrientation 
{
    HORIZONTAL = wx.wxHORIZONTAL, 
    VERTICAL   = wx.wxVERTICAL, 
    BOTH       = wx.wxBOTH 
}

function BoxSizer:BoxSizer ( nOrient )
    self.wxObj = wx.wxBoxSizer( nOrient )  
end

function BoxSizer:getwxObj( ... )
	return self.wxObj
end
...
function BoxSizer:add( oWidget, nProportion, nFlags, nBorder )
	isObject(oWidget) isNumber(nProportion) isNumber(nFlags) isNumber(nBorder)

    self:getwxObj():Add(oWidget:getwxObj(), nProportion or 0, nFlags or 0, nBorder or 0)
end
